...
uses
  ...
  Dialogs, ComCtrls, StdCtrls, Buttons, ExtCtrls, StrUtils_d7, ads_strg, uLogRead, Strutils,
  ...
  uTxt2xlsx_xml, ovcbase, ovcrlbl, Vcl.Mask (*JvExStdCtrls, JvEdit,*)
...
//    JvLabel1: TJvLabel;
    ...
    procedure tvViewExpanding(Sender: TObject; Node: TTreeNode; var AllowExpansion: Boolean);
    procedure tvViewMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure tvViewCollapsing(Sender: TObject; Node: TTreeNode; var AllowCollapse: Boolean);
    ...
    procedure chklstFMonlistKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    procedure ckReadselectMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    ...
    procedure ckReadselectKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    procedure sgLocationsSelectCell(Sender: TObject; ACol, ARow: Integer; var CanSelect: Boolean);
    procedure btnAddClick(Sender: TObject);
    procedure btnDelRowClick(Sender: TObject);
    procedure sgLocationsKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    procedure sgToCheckDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
    procedure sgToCheckKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
...
//    procedure rbDisplayClick(Sender: TObject);
    ...
    procedure splViewMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure bbRunMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure bbStopMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure bbShowallMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure bbSaveMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure ckDisplayMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure ckBuilddetailafterreadMouseMove(Sender: TObject;
      Shift: TShiftState; X, Y: Integer);
    procedure bbMNUMouseEnter(Sender: TObject);
    procedure bbMNUMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure bbResetMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure bbAddrowMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure bbClearRowMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure bbAddDirMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure bbCloseMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    ...
    procedure bbBuildMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure bbRefreshMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
  ...
  private
    ...
    function  ReadSelLogFolder(etRead: etReadtype; bBuildList : Boolean=False) : Boolean;
    ...
    function  SetFileInfo(idx : integer; asStr : Ansistring; bMont : boolean) : boolean;
    function  UpdFileInfo(idx : integer; asStr : Ansistring; bMont : boolean) : boolean;
    ...
    function  Rebuildfilechecklist(chlst : TChecklistbox; bFullpath : boolean) : boolean;
    function  PopulateCheckList(chklist: TChecklistbox; asFile : Ansistring; etRead: etReadtype; bBuildList : Boolean=False) : Boolean;
...
begin
  try
   ...
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'bbAddClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
 end;
end;
//------------------------------------------------------------------------------
...
begin
  try
    ...
    if not Assigned(sldirlist) then begin
      sldirlist := TStringlist.Create();
    end;

    while icnt < sgLocations.RowCount do begin
      if delbspace(sgLocations.Cells[0, icnt]) = '' then begin
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'builddirlist'+#13+#10+e.Message, mtWarning, [mbOK], 0);
      result := false;
   end;
  end;
end;
...
begin
  try
    ...
    if (slcmplst.Count > 0) and (sldirlist.count > 0) then begin
      if slcmplst.count = sldirlist.count then begin
        while (icnt < slcmplst.count) do begin
          if sldirlist.ValueFromIndex[icnt] <> slcmplst.ValueFromIndex[icnt] then begin
            bRebuild := true;
            break;
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'difdirlist'+#13+#10+e.Message, mtWarning, [mbOK], 0);
//      result := false;
   end;
  end;
end;
...
//    sgLocations.Options := [goFixedVertline, goRowSelect];
    ...
    if dlgOpendir.Execute then begin
      ...
      while idup <= sgLocations.RowCount do begin
        if delbspace(sgLocations.Cells[0, idup]) = delbspace(sdir) then begin
          bdup := true;
          break;
        end;
        inc(idup);
      ...
      if not bdup then begin
        if icnt = sglocations.RowCount then begin
        ...
        lbfinfo.Panels[1].Text := 'You selected a duplicate Directory. It was ignored.';
      end;

      if not builddirlist then begin
        lbfinfo.Panels[1].Text := 'NON Critical Isssue. If the issue persist report it.';
      end;
   ...
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'bbAddDirClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
 end;
end;
//------------------------------------------------------------------------------
procedure TfmLogFolder.bbAddDirMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  splViewMouseEnter(nil);
end;
//------------------------------------------------------------------------------
procedure TfmLogFolder.bbAddrowMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
   splViewMouseEnter(nil);
end;
...
//   if pgReadFiles.ActivePage = tsReadSelective then begin
     ...
     if ckReadselect.Items.Count > 0 then bbrun.Enabled := true;

//   end;
 except on
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'bbBuildClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
 end;
end;
procedure TfmLogFolder.bbBuildMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
   splViewMouseEnter(nil);
end;
...
begin
  try
    ...
    E : exception do begin
       MessageDlg('The following exception occurred in:'+#13+#10+'bbCancelClick'+#13+#10+e.Message, mtError, [mbOK], 0);
    end;
  end;
end;
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
procedure TfmLogFolder.bbCloseMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
   splViewMouseEnter(nil);
end;
...
//    if not SaveResults then
   ...
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'bbContinueClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
 end;
end;
//------------------------------------------------------------------------------
...
procedure TfmLogFolder.bbMNUMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
   splViewMouseEnter(nil);
end;
...
begin
 try
   ...
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'bbBuildflistClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
 end;
end;
//------------------------------------------------------------------------------
...
begin
 try
   if idx <> 0 then begin //[B 37]
     sgLocations.Cells[0,idx] := '';
     sgLocations.Cells[1,idx] := '';
   end;
 except on
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'bbDelRowClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
 end;
end;
//------------------------------------------------------------------------------
...
procedure TfmLogFolder.bbResetMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
   splViewMouseEnter(nil);
end;
...
//       bbBuild.Enabled := false;
       ...
       for icnt := 0 to ckReadselect.Items.Count-1 do begin
       ...
       if high(asselfilepath) >= 0 then begin
         ReadSelLogFolder(etReadsel);
//         bbBuild.Enabled := true;
         pgReadFiles.Enabled := true;
         bbShowall.Enabled := true;
       end else begin
         MessageDlg('Nothing was selected. Select a file to read', mtWarning, [mbOK], 0);
//         bbBuild.Enabled := true;
   ...
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'bbRunClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
//     bbBuild.Enabled := true;
     pgReadFiles.Enabled := true;
   end;
 end;
...
//------------------------------------------------------------------------------
procedure TfmLogFolder.bbRunMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
   splViewMouseEnter(nil);
end;
...
begin
  try
    ...
    e : Exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'bbSaveClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
    end;
  end;
end;
//------------------------------------------------------------------------------
procedure TfmLogFolder.bbSaveMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
   splViewMouseEnter(nil);
end;
...
begin
  ...
  if svOutput.Execute then begin
    ...
    if isEmpty(edSavetoo.Text) then begin
      edSavetoo.Text := IncludeTrailingPathDelimiter(ExtractFilePath(Application.ExeName))+'results.xml';
...
begin
 try
   ...
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'bbSellocClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
//     bbBuild.Enabled := true;
     pgReadFiles.Enabled := true;
   end;
 end;
...
//-----------------------------------------------------------------------------
procedure TfmLogFolder.bbStopMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
   splViewMouseEnter(nil);
end;
...
//-----------------------------------------------------------------------------
function  TfmLogFolder.BuildTree(etTyp : etLoad; bCrits : boolean=false) : Boolean;
var
...
begin
 try
   ...
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'cbFolderExit'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
 end;
end;
...
begin
 try
   ...
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'edaltsavelocKeyUp'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
 end;
end;
//******************************************************************************
procedure TfmLogFolder.edaltsavelocKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
 try
     ...
     if FileSize(ffollogs) > 0 then begin;
      ...
      if (etTyp = etnobystate) then begin
       ...
       while (not eof(ffollogs) ) or (eof(ffollogs)) and (iloopcnt = 0)  do begin
            ...
            if (prev = rc2flogs.folder) or (iflstfnd = 0) then begin//[153]
              if (prFile = rc2flogs.filename) or (iflstfnd = 0) then begin //[153]
                ...
                if (iflstfnd = 1) then begin //[134]
                  node[ic] := tvView.Items.Add(tmp ,rc2flogs.Folder +rc2flogs.filename);
                  node[ic] := tvView.Items.AddChild(node[ic],'['+Inttostr(rc2flogs.linenr)+']   '+ rc2flogs.Entry);
                end else
                if (iflst > 1) then begin
                  node[ic] := tvView.Items.Add(node[ic],'['+Inttostr(rc2flogs.linenr)+']   '+ rc2flogs.Entry);
                end;
              end
            end;
          end;
         ...
         if ix mod 700 = 0 then  begin //[B13]
           application.ProcessMessages;
           fmProgress.SetPogress(ix); //[B 29]
         end;
       end;
...
begin
 try
   if ckDisplay.Checked then begin
   ...
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'ckDisplayClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
 end;
end;
...
begin
 try
   ...
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'ckfilelistClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
 end;
end;
//------------------------------------------------------------------------------
procedure TfmLogFolder.ckfilelistDrawItem(Control: TWinControl; Index: Integer;            {[B 43]}
  Rect: TRect; State: TOwnerDrawState);
var
  Flags : Longint;
  idx   : integer;
...
begin
  with (Control as TCheckListBox) do begin
    if index > 0 then begin
      ...
      if (Containsstr(asstr, 'found issues') = true) then begin
...
begin
 try
   if key = vk_space then
    ...
    e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'ckReadselectKeyUp'+#13+#10+e.Message, mtWarning, [mbOK], 0);
...
begin
 try
  ...
  UpdateCntlabel;
    ...
    e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'ckReadselectMouseUp'+#13+#10+e.Message, mtWarning, [mbOK], 0);
...
//    bbBuild.Enabled := false;
    ...
    if iSetload <> 1 then begin
      irow                := 0;
      asUserprofile       := GetEnvironmentVariable('USERPROFILE');
      Settings            := TIniFile.Create(asUserprofile+'\saslogdisp.ini');
      irow                := 5;

      for icnt := 1 to irow do begin
        asVal  :=  Settings.ReadString('folderloc'+inttostr(icnt), 'folderloc'+inttostr(icnt), asVal);
        if DelMidSpaces(asVal) <> '' then begin
          sgLocations.RowCount := sgLocations.RowCount + 1;
...
begin
  try
   BuildFilterTree(etConvert);
    ...
    e: exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'lbConvClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
    end;
  end;
end;
//------------------------------------------------------------------------------
...
begin
  try
   BuildFilterTree(etCust);
    ...
    e: exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'lbCustClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
    end;
  end;
end;
//------------------------------------------------------------------------------
...
begin
  try
   BuildFilterTree(etError);
    ...
    e: exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'lberrClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
...
begin
  try
   BuildFilterTree(etInvalid);
    ...
    e: exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'lbInvaldClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
...
begin
 try
   ...
   e : Exception do begin
     MessageDlg('The following exception occurred in:'+#13+#10+'ReadAll1Click'+#13+#10+e.Message, mtWarning, [mbOK], 0);
//     bbBuild.Enabled := true;
     pgReadFiles.Enabled := true;
   end;
 end;
...
//------------------------------------------------------------------------------
function  TfmLogFolder.ReadLogFolder(asFile : Ansistring; etRead: etReadtype; bBuildList : Boolean=False) : Boolean;
...
//   while (irow < itrow) do begin;  //[195]
   for irow := 1 to itrow do begin;
     isfcnt    := 1;
     asval    := IncludeTrailingBackslash(ExcludeTrailingBackslash(sgLocations.Cells[0, irow]));
...
//           ftmploc  := floc;
           ...
           if etRead = etReadsel then begin
             if not bBuildList then begin
               for icnt := 0 to ckReadselect.Items.Count - 1 do begin
                 setlength(asFilePath, icnt+1);
                 if ckReadselect.Checked[icnt] then begin
           ...
           for icnt := 0 to high(asFilePath) do begin
            ...
            if etRead = etReadall then begin
...
//                JvxCheckListBox1.Items.Add(formatDateTime('yyyymmdd hh:mm AM/PM',FileDate(asFilePath[icnt])));
                ckfilelist.Items.Add(formatDateTime('yyyy/mm/dd hh:mm AM/PM',FileDate(asFilePath[icnt]))+' ' + asFilePath[icnt]);
            end else
            if etRead = etReadsel then begin
              if asFilePath[icnt] <> '' then
...
//                JvxCheckListBox1.Items.Add(formatDateTime('yyyymmdd hh:mm AM/PM',FileDate(asFilePath[icnt]))+' ' + asFilePath[icnt]);
                ckReadselect.Items.Add(formatDateTime('yyyy/mm/dd hh:mm AM/PM',FileDate(asFilePath[icnt]))+' ' + asFilePath[icnt]);
            end;
...
//              lbfinfo.Panels[0].Text     := '';
              ...
              if icnt mod 14 = 0 then  begin //[B 29]
                ...
                application.ProcessMessages;
                if lfRead.finalstatus > 0 then begin
                   ckFilelist.Items.Strings[ip] := '(Found Issues)'+ckFilelist.Items.Strings[ip];
                end;
     ...
     if not bBuildList then begin
       if sgLocations.Cells[0, irow] <> '' then  begin
...
//------------------------------------------------------------------------------
function  TfmLogFolder.BuildFileList(asFile : Ansistring; etRead: etReadtype) : Boolean;
var
  sr          : TSearchRec;
  bComplete   : Boolean;
  asVal       : Ansistring;
...
//------------------------------------------------------------------------------
function  TfmLogFolder.ReadSelLogFolder(etRead: etReadtype; bBuildList : Boolean=False) : Boolean;
var
  sr         : TSearchRec;
  bComplete  : Boolean;
  asVal      : Ansistring;
...
//   arFdets[ilen].FPath := asSelFilePath[icnt]
   ...
   lbFilesel.Caption := 'Files selected : '+inttostr(high(asselfilepath)+1);

   fmProgress.Setlabel('Reading Files. Please wait...', 1);
   fmProgress.SetMaxPogress(0);  //[156]

   if high(asSelfilepath) <> -1 then begin
    if bStop <>  true then begin
//       ftmploc  := floc;
       ...
       lfRead := TLogFileRead.create(aploc, asSelFilePath[0], true, pgRead, nil);
       lfRead.ClearCounters;
       Updatelabels;
       for icnt := 0 to high(asSelFilePath) do begin
          if bstop then break;
...
//          if not bBuildList then begin
  ...
  //          fmLogDisplay.ReadLogFile(true);
            if (high(asSelFilePath) > 14) and (icnt mod 14 = 0) then  begin //[B 29]
              fmProgress.SetPogress(icnt);
              Updatelabels;
              lbfinfo.Panels[0].Text := '';
              application.ProcessMessages;
...
//          end;  //if not bBuildList then begin
      ...
      if TimeDeltaInSeconds(dtst, now) = 0 then begin
         lbfinfo.Panels[0].Text  := ' Files read : ' + inttostr(ip) +
                                    '. Read and analyzed in : ' + FloatToStr(TimeDeltaInMSeconds(dtst, now))+'ms';
      end else begin
         lbfinfo.Panels[0].Text  := ' Files read : ' + inttostr(ip) +
                                   '. Read and analyzed in : ' + FloatToStr(TimeDeltaInSeconds(dtst, now))+'s';
      end;
...
//------------------------------------------------------------------------------
procedure TfmLogFolder.sgLocationsSelectCell(Sender: TObject; ACol,
...
begin
  ...
  sgToCheck.Enabled := false; //[B14]
   with sgToCheck do begin
     ...
     else if cells[0, sgrow]  = 'Math Oper.' then etTypes := etmathop
     else if cells[0, sgrow]  = 'No Obs in Data' then etTypes := etnobosdata
     else if cells[0, sgrow]  = 'No By State' then etTypes := etnobystate
...
//     else if cells[0, sgrow]  = 'Annotate Errors' then etTypes := etannoerr;
     ...
     if (Cells[1, sgRow] <> '') then begin
       if (StrToInt(Cells[1, sgRow]) > 0) then begin
...
begin
 try
   {With Sender as TStringGrid do begin
    ...
    e : Exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'sgToCheckDrawCell'+#13+#10+e.Message, mtWarning, [mbOK], 0);
    end;
 end;
end;
//------------------------------------------------------------------------------
...
(*  if (Key = VK_DOWN ) or (Key = VK_UP) then begin
   sgToCheckDblClick(nil);
    ...
    e : Exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'sgToCheckKeyUp'+#13+#10+e.Message, mtWarning, [mbOK], 0);
    end;
 end;
 end;
...
begin
  if bPin = true then begin
    bPin := false;
    splView.DisplayMode := svmOverlay;
    splView.Opened := false;
  end else
  if bPin = false then begin
    bPin := true;
...
begin
  try
   bbShowall.Enabled := false;
   if not BuildTree(etnone, false) then begin
     MessageDlg('The Result Tree could not be populated. '+#13+#10+
                'Restart the application and try again. If the problem persist, please report it. ', mtError, [mbOK], 0);
   end else begin
      bfilter := false;
...
//        bbBuild.Enabled := true;
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'bbShowallClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
      bbShowall.Enabled := false;
   end;
  end;
end;
...
begin
  try
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'tvViewMouseEnter'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
...
begin
  try
    if (tvView.Selected <> nil)  then begin
      if ((tvView.Selected.level > 1) and (tvView.Selected.Selected)) then begin
...
//          end;
      ...
      if (tvView.Selected.level = 1) and (tvView.Selected.Selected) and bfilter then begin //[166]
...
//          iline   := TryStrToInt(copy(sstr, idx+1, ((idx2-1)-idx)));
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'tvViewMouseUp'+#13+#10+e.Message, mtWarning, [mbOK], 0);
...
//        if aspre <> FieldByName('Logdest').AsString then begin;
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'FiltFolder'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
...
begin
  try
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'ExpandAll1Click'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
...
begin
  try
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'CollapseAll1Click'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
...
begin
  try
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'Unselectall1Click'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
...
//    end;
  ...
  E : Exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'CheckonClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
  end;
 end;
end;
//------------------------------------------------------------------------------
...
//    end;
    ...
    if tvView.Items.Count > 1 then begin
...
//        MessageDlg('Export of results failed.', mtError, [mbOK], 0);
       ...
       MessageDlg('There was no information found in the results to export.', mtInformation, [mbOK], 0);
    end;

  except on
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'SaveOutputClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
...
try
  ...
  if IsNotEmpty(dlgSaverdlst.FileName) then begin   {B 40]}
...
//  dlgSaverdlst.
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'SaveReadlist1Click'+#13+#10+e.Message, mtWarning, [mbOK], 0);
      fmLogFolder.FormStyle := fsStayOnTop;
      fmLogFolder.BringToFront;
   end;
  end;
...
try
  ...
  dlgOpenrdlst.Execute;
   ...
   sval  := '-1';
   if IsNotEmpty(dlgOpenrdlst.FileName) then begin   {B 40]}
     ...
     try
       ...
       if slfile.count > 0 then begin
         for icnt := 0 to slfile.Count-1 do begin
           sval1 := slfile.Strings[icnt];
  //         sval1 := slfile.Text;
           sval  := copy(sval1,0,1);
           sfile := copy(sval1,2, length(sval1));
           if sval = '1' then begin
...
//  ckReadselect.Items.LoadFromFile(dlgOpenrdlst.FileName);
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'OpenReadlist1Click'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
procedure TfmLogFolder.Panel2MouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
   splViewMouseEnter(nil);
end;
//------------------------------------------------------------------------------
procedure TfmLogFolder.Panel6MouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
   splViewMouseEnter(nil);
end;
...
//         inc(int);
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'SelectAll1Click'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
function  TfmLogFolder.SetFileInfo(idx : integer; asStr : Ansistring; bMont : boolean) : boolean;
begin
  try
...
//     arFInfo[idx].bMontr := bMont;
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'SetFileInfo'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
function  TfmLogFolder.UpdFileInfo(idx : integer; asStr : Ansistring; bMont : boolean) : boolean;
var
  I: Integer;
begin
  try
...
{    bfound := false;
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'chkFMonlistClick'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
...
//  if pgReadFiles.ActivePage = tsReadSelective then bbrun.Enabled := false;
  try
    ...
    if pgReadFiles.ActivePage = tsDetail then begin
...
//      bbSave.visible    := false;
      ...
      int := 0;
      if ckReadselect.Count > 0 then begin
        for icnt := 0 to ckReadselect.Items.Count-1 do begin
          if ckReadselect.State[icnt] = cbChecked then begin
            inc(int);
          end;
      ...
      if (int = 0 )and (not difdirlist) then begin
        ckReadselect.Items.Clear;
  //      ReadLogFolder('', etReadsel, true);
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'pgReadFilesChange'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
...
//    cdsfolders.EmptyDataSet;
   if FileExists(aploc+'follogs.dat') then begin
     try
       AssignFile(ffollogs,aploc+'follogs.dat') ;
       reset(ffollogs);
     finally
   ...
   for ic := 1 to sglocations.RowCount do begin;
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'ClearResult'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
...
begin
  try
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'Setlinecount'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
...
begin
  try
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'UpdateCntlabel'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
...
begin
  try
    ...
    else lbNotes.Color  := clBtnFace;

    if lbInfo.Caption      <> 'Info          : 0' then
      lbInfo.Color   := $0080FF80
    else lbInfo.Color   := clBtnFace;
...
begin
  try
   ...
   e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'Setuplocgrid'+#13+#10+e.Message, mtWarning, [mbOK], 0);
   end;
  end;
end;
//------------------------------------------------------------------------------
...
//    assaveloc := edaltsaveloc.Text;
    if IsEmpty(assaveloc) then begin
       asSaveloc := IncludeTrailingBackslash(ExtractFilePath(Application.ExeName));
    end;

    assignfile(ffollogs, aploc+'follogs.dat');
    reset(ffollogs);
    if filesize(ffollogs) > 0 then begin
      ashtmlfn :=  'LogCheck_result_'+GetEnvironmentVariable('USERNAME')+'_'+DateTimeToStr(now)+'.txt';
      ...
      try
        ...
        sllist.Add('Report Ran by : '+GetEnvironmentVariable('USERNAME') + ' on '+ DateTimeToStr(now));
        sllist.Add('---------------------------------------------------------------------------------------->');
        ...
        while not EOF(ffollogs) do begin
          read(ffollogs, rc1Flogs);
          sfilefol := '';
          if sfolder <> rc1flogs.folder then sfilefol := 'Folder :'+rc1flogs.Folder ;
          if sfile <> rc1flogs.filename then sfilefol := 'Folder :'+rc1flogs.Folder+' - File name : ' + rc1flogs.Filename;
          if sfilefol <> '' then begin
            sllist.Add( '');
            sllist.Add( '');
            sllist.Add(sfilefol);
          end;

          sllist.Add('    Type : '+rc1flogs.types + '['+inttostr(rc1flogs.linenr) +']   '+ rc1flogs.entry);
          sfile   := rc1flogs.Filename;
          sfolder := rc1flogs.Folder;
        end;
      ...
      MessageDlg('File Save to :'+#13+#10+assaveloc+ashtmlfn, mtInformation, [mbOK], 0);
//      lblocation.caption := 'File Save to : '+assaveloc+ashtmlfn;
    end;
    result := True;
  except on
    e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'SaveResults'+#13+#10+e.Message, mtWarning, [mbOK], 0);
      result := false;
    end;
  end;
end;
//------------------------------------------------------------------------------
function  TfmLogFolder.Rebuildfilechecklist(chlst : TChecklistbox; bFullpath : boolean) : boolean;
var
  icnt   : integer;
  sfpath : string;
  sfdtm  : string;
...
begin
  try
     setlength(arFlist, 1);
     for icnt := 0 to chlst.Items.Count-1 do begin
    ...
    e : exception do begin
      MessageDlg('The following exception occurred in:'+#13+#10+'Rebuildfilechecklist'+#13+#10+e.Message, mtWarning, [mbOK], 0);
      result := false;
    end;
  end;
end;
...
//------------------------------------------------------------------------------
function  TfmLogFolder.PopulateCheckList(chklist: TChecklistbox; asFile : Ansistring;
        ...
        if bStop <>  true then begin
         ...
         if isnotempty(asval) then begin
          ...
          (* if (ifol >= 0) and (icc  > 0 ) then begin
           ...
           if etRead = etReadsel then begin
             for icnt := 0 to chklist.Items.Count - 1 do begin
             setlength(asFilePath, icnt+1);
             if chklist.Checked[icnt] then begin
           ...
           for icnt := 0 to high(asFilePath) do begin
            if bstop then break;
              ...
              if asFilePath[icnt] <> '' then begin
                chklist.Items.Add(asFilePath[icnt]+ ' [ Filedate : '+DateTimeTostr(FileDate(asFilePath[icnt]))+']' );
...
//    assaveloc := IncludeTrailingBackslash(edaltsaveloc.Text);
    ...
    ashtmlfn :=  'LogCheck_result_'+GetEnvironmentVariable('USERNAME')+'_'+DateTimeToStr(now)+'.xlsx';
    ashtmlfn := ReplaceStr(ashtmlfn,'/','_');
    ashtmlfn := ReplaceStr(ashtmlfn,':','_');
    ashtmlfn := ReplaceStr(ashtmlfn,' ','_');
     ...
     ExcelApp.Application.Cells[3, 1].Value := 'Notes       : '+edNotes.text; *)
      ...
      if filesize(ffollogs) > 0 then begin
        try
          ...
          ExcelApp.Application.Workbooks[1].WorkSheets[2].Name := 'Header information';
